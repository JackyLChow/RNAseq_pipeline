...
png(paste0(parameters["output_folder", "value"], "PCA_on_", counts_, ".png"), width = 1200, height = 400)
print(plot_grid(title_, pc_plot_, ncol = 1, rel_heights = c(0.1, 1)))
dev.off()
checkpoint <- rbind(checkpoint, c(paste0("PCA_on_", counts_, ".png"), format(Sys.time(), "%Y-%m-%d %H:%M:%S %Z"))); write.csv(checkpoint, checkpoint_file, row.names = F)
